...
export const patientPortalAPI = {
  ...
  logGoalProgress: (goalId: string, data: {
    value: number;
    notes?: string;
  }) => api.post('/api/v1/patient/goal-progress', { goal_id: goalId, ...data }),
  
  /**
   * Get progress history for a specific goal
   */
  getGoalProgress: (goalId: string, params?: { 
    start_date?: string; 
    end_date?: string; 
    limit?: number; 
    offset?: number;
  }) => api.get('/api/v1/patient/goal-progress', { params: { goal_id: goalId, ...params } }),
