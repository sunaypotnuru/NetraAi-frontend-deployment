...
class ComplaintService {
  ...
  async submitComplaint(complaintData: ComplaintFormData): Promise<{
    ...
    try {
      ...
      const response = await fetch('/api/v1/compliance/complaints', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...payload,
          severity: complaintData.priority === 'urgent' ? 'Critical' : 
                   complaintData.priority === 'high' ? 'High' : 
                   complaintData.priority === 'medium' ? 'Medium' : 'Low',
          mdr_reportable: complaintData.affectsPatientCare || false,
          patient_harm: complaintData.affectsPatientCare ? 'Potential harm detected' : 'None',
          submitter_name: currentUser.name,
          submitter_email: currentUser.email,
          submitter_type: currentUser.type
        })
      });
