...
api.interceptors.request.use(async (config: InternalAxiosRequestConfig) => {
  // If bypass auth is enabled, send special demo headers (STRICTLY DEV ONLY)
  ...
  // Security Guard: Ensure bypass headers never leak to production
  if (!import.meta.env.DEV) {
    delete config.headers["X-Demo-Email"];
  ...
  if (session) {
    // Pre-emptive refresh if token is extremely close to expiring (within 60 seconds)
    const expiresAt = session.expires_at;
    const now = Math.floor(Date.now() / 1000);

    if (expiresAt && (expiresAt - now) < 60) {
      console.log("[API Interceptor] Token near expiry, forcing refresh...");
    ...
    console.warn(`[API Interceptor] NO SESSION for: ${config.url}`);
  }
  return config;
});
...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      console.log("[API Interceptor] 401 detected, attempting session refresh...");
      try {
        ...
        console.error("[API Interceptor] Session refresh failed during 401 retry:", err);
      }

      // If refresh fails, we might want to log out or redirect
      // But we'll let the component handle the error for now
    }
...
export const doctorAPI = {
  ...
  getDashboardStats: () => api.get("/api/v1/doctor/dashboard/stats"),
  getAvailability: () => api.get("/api/v1/doctor/availability"),
  ...
  getVitalsHistory: (patientId: string) => api.get(`/api/v1/doctor/patients/${patientId}/vitals`),

  // Analytics & Earnings (Doctor Portal)
  getAnalyticsOverview: (period: string = 'month') => api.get(`/api/v1/doctor/analytics/overview?period=${period}`),
  getRevenueAnalytics: (period: string = 'year') => api.get(`/api/v1/doctor/analytics/revenue?period=${period}`),
  getPatientAnalytics: (period: string = 'year') => api.get(`/api/v1/doctor/analytics/patients?period=${period}`),
  getEarningsSummary: (period: string = 'month') => api.get(`/api/v1/doctor/earnings/summary?period=${period}`),
  getTransactions: (params?: Record<string, string>) => api.get('/api/v1/doctor/transactions', { params }),
};

// Vitals endpoints
export const vitalsAPI = {
  getVitalsHistory: (patientId: string) => api.get(`/api/v1/doctor/patients/${patientId}/vitals`),
  addVitalLog: (patientId: string, data: Record<string, unknown>) => api.post(`/api/v1/doctor/patients/${patientId}/vitals`, data),
};
...
export const adminAPI = {
  ...
  getUsers: (params?: Record<string, unknown>) => api.get("/api/v1/admin/users", { params }),
  getPatients: (params?: Record<string, unknown>) => api.get("/api/v1/admin/patients", { params }),
  getPatient: (id: string) => api.get(`/api/v1/admin/patients/${id}`),
  getDoctors: (params?: Record<string, unknown>) => api.get("/api/v1/admin/doctors", { params }),
  getDoctor: (id: string) => api.get(`/api/v1/admin/doctors/${id}`),
  getAppointments: (params?: Record<string, unknown>) => api.get("/api/v1/admin/appointments", { params }),
  getAppointment: (id: string) => api.get(`/api/v1/admin/appointments/${id}`),
  getScans: (params?: Record<string, unknown>) => api.get("/api/v1/admin/scans", { params }),
  getPayments: (params?: Record<string, unknown>) => api.get("/api/v1/admin/payments", { params }),
  getRefunds: (params?: Record<string, unknown>) => api.get("/api/v1/admin/refunds", { params }),
  getReviews: (params?: Record<string, unknown>) => api.get("/api/v1/admin/reviews", { params }),
  ...
  // Admin Security
  getSecurityStats: () => api.get("/api/v1/admin/security/stats"),
  getSecuritySessions: () => api.get("/api/v1/admin/security/sessions"),
  getSecurityLogs: (limit?: number) => api.get("/api/v1/admin/security/logs", { params: { limit } }),
  forceTerminateUserSessions: (userId: string) => api.post(`/api/v1/admin/security/sessions/${userId}/terminate`),
  // Admin Config
  getFeatures: () => api.get("/api/v1/admin/config/features"),
  toggleFeature: (featureName: string, enabled: boolean) => api.post("/api/v1/admin/config/feature-toggle", { feature_name: featureName, enabled }),
  updateConfiguration: (key: string, value: any, description?: string) => api.put("/api/v1/admin/config", { key, value, description }),
  getConfiguration: () => api.get("/api/v1/admin/config"),
  // Lightweight contact list for admin messaging
  getMessageContacts: () => api.get("/api/v1/admin/message-contacts"),
};

export const blogsAPI = {
  getBlogs: (params?: Record<string, unknown>) => api.get("/api/v1/blogs", { params }),
  getBlog: (id: string) => api.get(`/api/v1/blogs/${id}`),
  createBlog: (data: Record<string, unknown>) => api.post("/api/v1/blogs", data),
  updateBlog: (id: string, data: Record<string, unknown>) => api.put(`/api/v1/blogs/${id}`, data),
  deleteBlog: (id: string) => api.delete(`/api/v1/blogs/${id}`),
};

export const complianceAPI = {
  // FDA APM
  getFDAModels: () => api.get("/api/v1/compliance/fda-apm/models"),
  getFDAMetrics: (model: string, hours = 24) => api.get(`/api/v1/compliance/fda-apm/metrics/${model}?hours=${hours}`),
  getFDAAlerts: () => api.get("/api/v1/compliance/fda-apm/alerts"),

  // SOC 2
  getSOC2Statistics: () => api.get("/api/v1/compliance/soc2/statistics"),
  getSOC2Controls: (category = "all") => api.get(`/api/v1/compliance/soc2/controls?category=${category}`),
