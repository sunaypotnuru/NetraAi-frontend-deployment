...
import { authAPI, patientAPI, profileAPI } from "./api";
...
export const useAuthStore = create<AuthState>((set, get) => ({
    ...
    try {
      ...
      try {
        ...
        if (pRes.data) {
          ...
          // Explicitly update profile with registration data as a safety net
          // (Triggers might miss some fields or have minor delays)
          if (role === 'doctor') {
            ...
            if (updatedProfile) {
              profileData = Array.isArray(updatedProfile) ? updatedProfile[0] : updatedProfile;
            }
          }
        }
      } catch (e) {
  ...
  loadUser: async () => {
    ...
    try {
      ...
      // Refresh session if near expiration (less than 10 minutes left)
