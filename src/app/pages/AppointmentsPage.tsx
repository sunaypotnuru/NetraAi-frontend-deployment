...
export default function AppointmentsPage() {
  ...
  const exportCSV = async () => {
    ...
    const worksheet = workbook.addWorksheet(t('patient.appointments.sheet_name', "Appointments"));
    
    // Add headers
    worksheet.columns = [
      { header: t('common.date', "Date"), key: 'date', width: 15 },
      { header: t('patient.appointments.time', "Time"), key: 'time', width: 12 },
      { header: t('common.doctor', "Doctor"), key: 'doctor', width: 20 },
      { header: t('common.specialty', "Specialty"), key: 'specialty', width: 20 },
      { header: t('patient.appointments.status', "Status"), key: 'status', width: 15 },
      { header: t('common.type', "Type"), key: 'type', width: 15 }
    ];
    
    // Add data
    allAppointments.forEach((a) => {
      worksheet.addRow({
        date: safeFormat(a.scheduled_at, "MMM d, yyyy"),
        time: safeFormat(a.scheduled_at, "h:mm a"),
        doctor: a.profiles_doctor?.name || '-',
        specialty: a.profiles_doctor?.specialty || '-',
        status: t(`patient.appointments.status_${a.status}`, a.status).toUpperCase(),
        type: a.consultation_type || a.type || t('patient.appointments.in_person', 'In-person')
      });
    });
    ...
    a.href = url;
    a.download = 'appointments_history.csv';
    a.click();
