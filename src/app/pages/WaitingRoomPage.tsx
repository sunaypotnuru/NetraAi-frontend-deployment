...
export default function WaitingRoomPage() {
    ...
    useEffect(() => {
        ...
        const fetchQueueStatus = async () => {
            try {
                ...
                if (response.data) {
                    ...
                    setWaitTime((response.data.estimated_wait_minutes || 5) * 60);
        ...
        // Use WebSocket for real-time updates instead of Supabase
        let notificationConn: any = null;

        const handleWebSocketMessage = (data: any) => {
            console.log('[WaitingRoom] WebSocket message received:', data);
            
            // Check if this notification is for this appointment
            if (data.notification_type === 'patient_called' && data.payload?.appointment_id === appointmentId) {
                setIsCallReady(true);
                toast.success(t('patient.waiting.doctor_ready', "🎉 The doctor is ready for you!"), {
                    duration: 10000,
                    action: {
        ...
        const setupSocket = async () => {
            try {
                notificationConn = await connect('notifications');
                notificationConn.on('notification', handleWebSocketMessage);
            } catch (err) {
                console.error('[WaitingRoom] Socket setup failed:', err);
            }
        };
        ...
        return () => {
            if (notificationConn) {
                notificationConn.off('notification', handleWebSocketMessage);
            }
        };
