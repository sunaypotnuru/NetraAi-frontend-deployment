  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">{t("patient.parkinsons.title", "Parkinson's Voice Analysis")}</h1>
      </div>
      ...
      <Card className="p-8 shadow-xl bg-white/70 dark:bg-slate-900/50 backdrop-blur-md border border-slate-200/50 dark:border-white/10 transition-all duration-300">
        <div className="flex flex-col items-center gap-6">
          ...
          {!audioBlob && !recording && (
            <Button onClick={startRecording} size="lg" className="rounded-full w-56 h-16 text-lg bg-patient-primary hover:bg-teal-700 text-white font-bold shadow-lg hover:shadow-teal-500/20 active:scale-95 transition-all duration-300">
          ...
          {recording && (
            <div className="flex flex-col items-center gap-4 w-full">
              ...
              <Button onClick={stopRecording} size="lg" variant="destructive" className="rounded-full w-48 h-12 flex items-center justify-center shadow-[0_0_15px_rgba(239,68,68,0.4)] active:scale-95 transition-all duration-300">
          ...
          {audioBlob && !analyzing && !recording && (
            <div className="flex flex-col items-center gap-6 w-full mt-4">
              <audio controls src={URL.createObjectURL(audioBlob)} className="w-full max-w-sm rounded-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700" />
              <div className="flex gap-4">
                <Button onClick={uploadAndAnalyze} size="lg" className="bg-teal-600 hover:bg-teal-700 text-white w-56 shadow-md transition-all duration-300">
                  <Activity className="w-5 h-5 mr-2" /> {t("common.analyze_tremors", "Analyze Vocal Tremors")}
                </Button>
                <Button onClick={() => setAudioBlob(null)} variant="outline" size="lg" className="w-32 border-slate-200 dark:border-slate-850 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all duration-300">
          ...
          {analyzing && (
            <div className="flex flex-col items-center gap-4 text-patient-primary w-full mt-4">
              ...
              <p className="font-bold text-lg animate-pulse text-teal-600 dark:text-teal-400">{t("patient.parkinsons.analyzing", "Running Vocal Frequency Map...")}</p>
            </div>
          )}

          {error && (
            <Alert variant="destructive" className="w-full max-w-md mt-4 bg-red-50/90 dark:bg-red-950/40 border border-red-200 dark:border-red-800 text-sm">
      ...
      <AnimatePresence>
        {result && (
          <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4 }}>
            <Card className="p-8 shadow-xl bg-white/70 dark:bg-slate-900/50 backdrop-blur-md border border-slate-200/50 dark:border-white/10 transition-all duration-300 mt-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="p-6 bg-red-50/50 dark:bg-red-950/30 rounded-xl border border-red-100/50 dark:border-red-900/50">
                  <p className="text-red-800 dark:text-red-300 font-medium mb-2 text-sm">{t("patient.scan.biomarker_risk", "Biomarker Risk Score")}</p>
                  <p className="text-4xl font-black text-red-900 dark:text-red-200">{(result.risk_score * 100).toFixed(0)}%</p>
                </div>
                <div className="p-6 bg-green-50/50 dark:bg-green-950/30 rounded-xl border border-green-100/50 dark:border-green-900/50">
                  <p className="text-green-800 dark:text-green-300 font-medium mb-2 text-sm">{t("patient.scan.ai_confidence", "AI Confidence")}</p>
                  <p className="text-4xl font-black text-green-900 dark:text-green-200">{(result.confidence * 100).toFixed(0)}%</p>
                </div>
                <div className="p-6 bg-blue-50/50 dark:bg-blue-950/30 rounded-xl border border-blue-100/50 dark:border-blue-900/50 flex flex-col justify-center">
                  <p className="text-blue-800 dark:text-blue-300 font-medium mb-2 text-sm">{t("patient.scan.recommendation", "Recommendation")}</p>
                  <p className="text-lg font-bold text-blue-900 dark:text-blue-200 leading-snug">{result.recommendation}</p>
                </div>
              </div>
            </Card>
          </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
        <Card className="p-8 bg-gradient-to-r from-teal-500/5 to-indigo-500/5 border-none shadow-lg transition-all duration-300">
          <div className="flex flex-col md:flex-row gap-8 items-center">
            <div className="w-20 h-20 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-2xl shadow-md flex items-center justify-center flex-shrink-0">
          ...
          <Card className="p-6 h-full bg-white/70 dark:bg-slate-900/50 backdrop-blur-md border border-slate-200/50 dark:border-white/10 shadow-lg transition-all duration-300">
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={idx} className="p-3 bg-white/50 dark:bg-slate-950/40 rounded-lg border border-slate-100 dark:border-slate-800/80 hover:border-teal-500/30 dark:hover:border-teal-500/30 transition-all duration-300">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
            <Card className="p-6 bg-white/70 dark:bg-slate-900/50 backdrop-blur-md border border-slate-200/50 dark:border-white/10 shadow-lg transition-all duration-300">
              ...
              <div className="grid grid-cols-1 gap-4">
                <div className="flex items-start gap-4 p-4 bg-purple-500/10 dark:bg-purple-950/20 rounded-xl">
                  <ArrowRight className="w-5 h-5 text-purple-500 mt-1 flex-shrink-0" />
                  <div>
                    <h4 className="font-bold text-slate-900 dark:text-white text-sm">{t('patient.parkinsons.treat1', 'LSVT LOUD® Therapy')}</h4>
                    <p className="text-xs text-slate-600 dark:text-slate-400 mt-0.5 leading-relaxed">{t('patient.parkinsons.treat1_desc', 'Evidence-based speech treatment to improve vocal loudness and clarity.')}</p>
                  </div>
                </div>
                <div className="flex items-start gap-4 p-4 bg-blue-500/10 dark:bg-blue-950/20 rounded-xl">
                  <ArrowRight className="w-5 h-5 text-blue-500 mt-1 flex-shrink-0" />
                  <div>
                    <h4 className="font-bold text-slate-900 dark:text-white text-sm">{t('patient.parkinsons.treat2', 'Dopaminergic Medication')}</h4>
                    <p className="text-xs text-slate-600 dark:text-slate-400 mt-0.5 leading-relaxed">{t('patient.parkinsons.treat2_desc', 'Levodopa and other medications to manage motor and vocal symptoms.')}</p>
                  </div>
                </div>
              </div>
            </Card>
