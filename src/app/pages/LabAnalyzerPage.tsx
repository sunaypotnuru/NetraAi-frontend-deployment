        ...
        <div className="min-h-screen pt-3 pb-12 px-6 bg-background">
            <div className="max-w-5xl mx-auto space-y-8">
                <div className="text-center space-y-4 max-w-2xl mx-auto">
                    ...
                    <h1 className="text-foreground tracking-tight">{t('patient.lab.title', "AI Lab Report Analyzer")}</h1>
                ...
                <div className="grid lg:grid-cols-2 gap-8 items-start">
                    {/* Left: Uploader */}
                    <Card className="p-8 border border-[#0D9488]/20 shadow-xl shadow-teal-900/5 bg-card border-border shadow-xl shadow-teal-900/5 rounded-3xl relative overflow-hidden">
                        <div className="absolute top-0 right-0 w-32 h-32 bg-teal-50 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2" />

                        <h3 className="text-foreground mb-4 relative z-10 flex items-center gap-2">
                            ...
                            <AnimatePresence mode="popLayout">
                                {file ? (
                                    ...
                                    >
                                        ...
                                        <div>
                                            <h4 className="font-bold text-foreground max-w-[200px] truncate">{file.name}</h4>
                                            <p className="text-xs font-medium text-gray-500">{formatBytes(file.size)} • {t('patient.lab.ready_to_scan', "Ready to scan")}</p>
                                        </div>
                                    </motion.div>
                                ) : (
                                    <motion.div className="flex flex-col items-center text-muted-foreground space-y-4 group-hover:text-[#0D9488] transition-colors relative z-10">
                                        ...
                                        <div>
                                            <p className="font-bold text-foreground group-hover:text-[#0D9488]">{t('patient.lab.select_file', "Select PDF or Image")}</p>
                        ...
                        <AnimatePresence mode="popLayout">
                            {analyzeMutation.isPending ? (
                                ...
                                <motion.div
                                    ...
                                    className="space-y-4"
                                >
                                    <Card className="p-6 bg-card rounded-3xl border border-border shadow-sm">
                                        ...
                                        {results.metrics && results.metrics.length > 0 ? (
                                            <div className="grid gap-3">
                                                {results.metrics.map((metric: LabMetric, i: number) => (
                                                    <div key={i} className="flex justify-between items-center p-4 rounded-2xl bg-muted/50 border-border hover:bg-card hover:shadow-md transition-all">
                                ...
                                <div className="bg-muted/30 border-2 border-dashed border-border rounded-3xl flex flex-col items-center justify-center text-gray-400 p-8 text-center">
                                    <FileText className="w-16 h-16 mb-4 opacity-20" />
                                    <p className="text-foreground font-medium text-lg">{t('patient.lab.awaiting_report', "Awaiting Lab Report")}</p>
