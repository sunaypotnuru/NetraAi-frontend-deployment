...
import { useAuthStore, usePresenceStore } from "../../lib/store";
import api, { messagesAPI, adminAPI } from "../../lib/api";
...
export default function MessagesPage() {
  ...
  useEffect(() => {
    ...
    const setupWS = async () => {
      try {
        const conn = await connect("messages");
        if (!isSubscribed) return;

        // Join the conversation room
        send("messages", "join_conversation", { conversation_id: convId });

        // Register handlers
        const handleNewMsg = (data: any) => {
          if (data.message && (data.message.sender_id === selectedConversation.partner_id || data.message.recipient_id === selectedConversation.partner_id)) {
            setMessages(prev => {
        ...
        const handleTyping = (data: any) => {
          if (data.user_id === selectedConversation.partner_id && data.conversation_id === convId) {
        ...
        const handleRead = (data: any) => {
          if (data.conversation_id === convId && data.reader_id === selectedConversation.partner_id) {
            setMessages(prev => prev.map(m => m.sender_id === user.id ? { ...m, read: true } : m));
          }
        };

        conn.on("new_message", handleNewMsg);
        conn.on("user_typing", handleTyping);
        conn.on("message_read", handleRead);

        return () => {
          conn.off("new_message", handleNewMsg);
          conn.off("user_typing", handleTyping);
          conn.off("message_read", handleRead);
          send("messages", "leave_conversation", { conversation_id: convId });
        };
      } catch (err) {
        console.error("[Messages] WebSocket setup failed:", err);
      }
    };

    const cleanupPromise = setupWS();
  ...
  const loadConversations = async () => {
    try {
      ...
      const convs = Array.isArray(response.data) ? response.data : (response.data?.conversations || response.data?.data || []);
  ...
  }, [selectedConversation, user?.id, send]);
  ...
  const sendHTTPFallback = async (text: string) => {
  ...
  const sendMessage = async (overrideText?: string) => {
    ...
    if (!text || !selectedConversation || !user?.id) return;
    ...
    try {
      ...
      if (isConnected) {
        try {
          ...
          console.warn("[Messages] WS send failed, attempting HTTP fallback:", wsError);
          await sendHTTPFallback(text);
        }
      } else {
        console.log("[Messages] WebSocket disconnected, using HTTP fallback.");
        await sendHTTPFallback(text);
      }
    } catch {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const convId = [user.id, selectedConversation.partner_id].sort().join(":");
      const messageContent = isImage ? `[IMAGE_ATTACHMENT]${file.name}||${publicUrl}` : `[FILE_ATTACHMENT]📎 ${file.name}||${publicUrl}`;
      ...
      if (isConnected) {
        try {
          ...
          console.warn("[Messages] Attachment WS send failed, using HTTP fallback:", wsError);
        ...
        console.log("[Messages] WebSocket disconnected, sending attachment via HTTP.");
        await messagesAPI.sendMessage({
          recipient_id: selectedConversation.partner_id,
          content: messageContent,
        });
  ...
  const loadAvailableContacts = async () => {
    ...
    try {
      ...
      if (user?.role === "patient") {
        // Returns a plain array of verified doctors
        const response = await api.get("/api/v1/doctors");
        contacts = Array.isArray(response.data) ? response.data : [];
      } else if (user?.role === "doctor") {
        // Returns a plain array of patients with appointments
        const response = await api.get("/api/v1/doctor/patients");
        contacts = Array.isArray(response.data) ? response.data : [];
      } else if (user?.role === "admin") {
        // Use the lightweight dedicated endpoint – avoids heavy per-user auth-admin calls
        const response = await adminAPI.getMessageContacts();
        contacts = response.data?.contacts || [];
      } else {
        // Unknown role – nothing to load
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="relative shrink-0">
                      ...
                      {!!globalOnlineUsers[conv.partner_id] && (
                        <div className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 border-2 border-white rounded-full" />
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-0.5">
                        <p className="font-bold text-sm text-[#0F172A] truncate">{conv.partner_name || t('common.unknown', "Unknown")}</p>
                        {conv.unread_count > 0 && (
              ...
              <>
                ...
                <div className="px-4 py-3 border-b border-gray-100 flex items-center gap-3 bg-white shrink-0">
                  ...
                  <div className="relative">
                    ...
                    {globalOnlineUsers[selectedConversation.partner_id] && (
                      <div className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-green-500 border-2 border-white rounded-full" />
                    )}
                  </div>
                  <div>
                    <p className="font-semibold text-[#0F172A] text-sm">{selectedConversation.partner_name || t('common.unknown', "Unknown")}</p>
                    <p className="text-xs text-[#64748B]">
                      {partnerTyping ? (
                        <span className="text-[#0D9488] animate-pulse">{t('patient.messages.typing', "typing...")}</span>
                        ) : globalOnlineUsers[selectedConversation.partner_id] ? t('common.online', "Online") : t('common.offline', "Offline")}
                    </p>
                  </div>
                </div>
                        ...
                        <div className="relative max-w-[70%]">
                          ...
                          {msg.reply_to && (
                            <div className={`px-3 py-1.5 mb-1 rounded-lg text-xs border-l-2 border-[#0D9488] bg-gray-50 text-gray-500 truncate`}>
                              <span className="font-medium">{msg.reply_to.sender_id === user?.id ? t('common.you', "You") : (selectedConversation.partner_name || t('common.unknown', "Unknown"))}:</span>{" "}
                    ...
                    >
                      <div className="flex-1 border-l-2 border-[#0D9488] pl-2">
                        <p className="text-[10px] text-[#0D9488] font-medium">
                          {replyTo.sender_id === user?.id ? t('common.you', "You") : (selectedConversation.partner_name || t('common.unknown', "Unknown"))}
                        </p>
          ...
          <div className="mt-4">
            {loadingContacts ? (
              ...
              <div className="text-center py-8 text-red-500">
                <p className="text-sm font-semibold">{t('patient.messages.contacts_error', "Could not load contacts")}</p>
                <p className="text-xs mt-1 text-gray-500">{contactsError}</p>
                <button
                  onClick={loadAvailableContacts}
                  className="mt-3 text-xs text-[#0D9488] underline hover:no-underline"
