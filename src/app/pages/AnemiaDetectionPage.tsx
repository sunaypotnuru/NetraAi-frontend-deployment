  ...
  return (
    <div className="min-h-screen pt-3 pb-12 px-6">
      <div className="max-w-5xl mx-auto">
        ...
        >
          <h1 className="text-foreground mb-2">{t('patient.scan.title', 'AI Anemia Detection')}</h1>
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            {/* Upload Section */}
            <Card className="p-8">
              <h2 className="text-foreground mb-6">{t('patient.scan.upload_image', 'Upload Image')}</h2>

              {!previewUrl ? (
                <div>
                  <label
                    htmlFor="file-upload"
                    className="block border-border hover:border-patient-primary transition-colors cursor-pointer bg-card shadow-sm"
                  >
                ...
                <div>
                  <div className="relative rounded-xl overflow-hidden mb-4 border border-gray-100 shadow-sm bg-black flex items-center justify-center h-64">
                    <img
                      src={previewUrl}
                      alt="Selected"
                    ...
                    <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
                      ...
                      <div className="absolute top-4 left-4 w-6 h-6 border-t-2 border-l-2 border-emerald-500 rounded-tl opacity-75" />
                      <div className="absolute top-4 right-4 w-6 h-6 border-t-2 border-r-2 border-emerald-500 rounded-tr opacity-75" />
                      <div className="absolute bottom-4 left-4 w-6 h-6 border-b-2 border-l-2 border-emerald-500 rounded-bl opacity-75" />
                      <div className="absolute bottom-4 right-4 w-6 h-6 border-b-2 border-r-2 border-emerald-500 rounded-br opacity-75" />

                      {/* Rotating scanning crosshairs/circle */}
                      <motion.div
                        className="w-44 h-44 border border-dashed border-emerald-500/30 rounded-full"
                        animate={{ rotate: 360 }}
                        transition={{ duration: 20, repeat: Infinity, ease: "linear" }}
                      />
                      <motion.div
                        className="w-32 h-32 border border-emerald-500/10 rounded-full absolute"
                        animate={{ scale: [1, 1.05, 1] }}
                        transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
                      />

                      {/* Medical targeting dot */}
                      <div className="w-1.5 h-1.5 bg-emerald-500/60 rounded-full absolute" />
                    </div>

                    {analyzing && (
                      <>
                        {/* Scanning green glow sweep */}
                        <motion.div
                          className="absolute inset-x-0 h-16 bg-gradient-to-b from-transparent to-emerald-500/20 pointer-events-none"
                          animate={{
                            top: ['0%', '75%', '0%']
                          }}
                          transition={{
                        ...
                        />
                        {/* Glowing neon green laser bar */}
                        <motion.div
                          className="absolute inset-x-0 h-1 bg-emerald-400 shadow-[0_0_15px_#10b981,0_0_30px_#10b981] pointer-events-none"
                          animate={{
                            top: ['0%', '100%', '0%']
                          }}
                          transition={{
                    ...
                    {qualityStatus && qualityStatus !== 'checking' && (
                      <div className={`absolute bottom-0 left-0 right-0 p-3 text-sm font-medium backdrop-blur-md flex items-center justify-center gap-2 z-20
              ...
              {analyzing && (
                <div className="mt-6">
                  <p className="text-sm text-muted-foreground mb-2">{t('patient.scan.analyzing_image', 'Analyzing image...')}</p>
            ...
            {/* Results Section */}
            <Card className="p-8">
              <h2 className="text-foreground mb-6">{t('patient.scan.analysis_results', 'Analysis Results')}</h2>
              ...
              {mutation.isError ? (
                <div className="flex flex-col items-center justify-center h-64 text-center">
                  ...
                  <h3 className="text-foreground mb-2">{t('patient.scan.analysis_err', 'Analysis Failed')}</h3>
                ...
                >
                  ...
                  <motion.div
                    ...
                    className="text-center p-6 rounded-xl bg-gradient-to-br from-[#F43F5E]/10 to-[#C0392B]/10 dark:from-[#F43F5E]/20 dark:to-[#C0392B]/20 border-2 border-[#F43F5E]/20 dark:border-[#F43F5E]/30"
                  >
                    <h3 className="text-muted-foreground mb-2">{t('patient.scan.prediction', 'Prediction')}</h3>
                  ...
                  >
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-sm font-medium text-foreground">{t('patient.scan.confidence', 'Confidence')}</span>
                  ...
                  <motion.div
                    ...
                    className="p-4 bg-[#0EA5E9]/10 dark:bg-[#0EA5E9]/20 rounded-lg border border-[#0EA5E9]/20 dark:border-[#0EA5E9]/30"
                  >
                    <p className="text-muted-foreground mb-1">{t('patient.scan.estimated_hb', 'Estimated Hemoglobin')}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-12 pb-12">
            ...
            <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              <Card className="p-6 h-full bg-card border-border shadow-md">
                ...
                <Card className="p-6 glass-card border-none shadow-xl bg-gradient-to-br from-blue-500/5 to-transparent">
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    <div className="flex items-start gap-4 p-4 bg-blue-500/10 rounded-xl">
                      ...
                      <div>
                        <h4 className="font-bold text-foreground">{t('patient.anemia.step1', 'Complete Blood Count (CBC)')}</h4>
                    ...
                    <div className="flex items-start gap-4 p-4 bg-green-500/10 rounded-xl">
                      ...
                      <div>
                        <h4 className="font-bold text-foreground">{t('patient.anemia.step2', 'Tele-Consultation')}</h4>
