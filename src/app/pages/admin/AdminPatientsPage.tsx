...
export default function AdminPatientsPage() {
    ...
    if (isLoading) {
        return (
            <div className="space-y-6">
                ...
                <Card className="p-0 border border-gray-200 dark:border-white/10 bg-white dark:bg-[#1E293B]">
                    <div className="p-8 space-y-4">
                        {[1, 2, 3, 4, 5].map(i => <Skeleton key={i} height={60} variant="rounded" sx={{ bgcolor: 'rgba(255,255,255,0.05)' }} />)}
                    </div>
                </Card>
            </div>
        );
    ...
    if (error) {
        console.error("[AdminPatientsPage] Error loading patients:", error);
        return (
            <div className="p-12 text-center bg-red-50 dark:bg-red-500/10 rounded-2xl border border-red-100 dark:border-red-500/20">
                ...
                <h3 className="text-lg font-bold text-red-900 dark:text-red-400">{t("admin.patients.failed_load", "Failed to load patients")}</h3>
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            <div>
                <h1 className="text-3xl font-bold text-foreground mb-1">{t("admin.patients.title", "Patient Directory")}</h1>
                <p className="text-muted-foreground">{t("admin.patients.subtitle", "View and manage patient accounts and records")}</p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
                ...
                <Button variant="outline" className="bg-white dark:bg-white/5 border-gray-200 dark:border-white/10 gap-2 text-[#64748B] dark:text-gray-400">
                    ...
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-white/5">
                            {filteredPatients.map((patient: Patient) => (
                                <tr key={patient.id} className="hover:bg-gray-50/80 dark:hover:bg-white/5 transition-colors">
                                    <td className="p-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p className="font-bold text-foreground">{patient.full_name || patient.name || t("common.anonymous_patient", "Anonymous Patient")}</p>
                                                <p className="text-xs text-muted-foreground font-mono mt-0.5">{patient.id.slice(0, 8)}...</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="p-4">
                                        <div className="space-y-1">
                                            <p className="text-sm text-foreground flex items-center gap-1.5"><Mail className="w-3.5 h-3.5 text-muted-foreground" /> {patient.email}</p>
                                            <p className="text-sm text-foreground flex items-center gap-1.5"><Phone className="w-3.5 h-3.5 text-muted-foreground" /> {patient.phone || t("common.no_phone", "No phone")}</p>
                                        </div>
                                    </td>
                                    <td className="p-4">
                                        <p className="text-sm text-foreground flex items-center gap-1.5"><MapPin className="w-4 h-4 text-muted-foreground" /> {patient.city || t("common.unknown", "Unknown")}</p>
                                    </td>
                                    <td className="p-4">
