...
export default function ConfigurationPage() {
  ...
  const loadConfiguration = async () => {
    try {
      ...
      const dbConfig = response.data?.configuration || {};
      
      setConfig({
        features: {
          aiNurse: dbConfig.ai_nurse_enabled ?? true,
          chatbot: dbConfig.mental_health_chatbot_enabled ?? true,
          emergencyServices: dbConfig.emergency_services_enabled ?? true,
          semanticSearch: dbConfig.semantic_search_enabled ?? true,
          mentalHealth: dbConfig.mental_health_enabled ?? true,
        },
        system: {
          sessionTimeout: dbConfig.session_timeout_minutes ?? 30,
          maxLoginAttempts: dbConfig.failed_login_attempts_limit ?? 5,
          rateLimit: dbConfig.rate_limit_per_minute ?? 100,
          maintenanceMode: dbConfig.maintenance_mode ?? false,
        },
        notifications: {
          emailEnabled: dbConfig.email_notifications_enabled ?? true,
          smsEnabled: dbConfig.sms_notifications_enabled ?? false,
          emailProvider: dbConfig.email_provider ?? "SMTP",
          smsProvider: dbConfig.sms_provider ?? "Twilio",
        },
      });
    } catch (error) {
      console.error("Failed to load system configuration:", error);
      toast.error("Failed to load system configuration from server.");
    }
  };
  ...
  const updateNotificationValue = (setting: keyof SystemConfig["notifications"], value: string) => {
    setConfig({
      ...config,
      notifications: {
        ...config.notifications,
  ...
  const saveConfiguration = async () => {
    ...
    try {
      const configMap = {
        "ai_nurse_enabled": config.features.aiNurse,
        "mental_health_chatbot_enabled": config.features.chatbot,
        "emergency_services_enabled": config.features.emergencyServices,
        "semantic_search_enabled": config.features.semanticSearch,
        "mental_health_enabled": config.features.mentalHealth,

        "session_timeout_minutes": config.system.sessionTimeout,
        "failed_login_attempts_limit": config.system.maxLoginAttempts,
        "rate_limit_per_minute": config.system.rateLimit,
        "maintenance_mode": config.system.maintenanceMode,

        "email_notifications_enabled": config.notifications.emailEnabled,
        "sms_notifications_enabled": config.notifications.smsEnabled,
        "email_provider": config.notifications.emailProvider,
        "sms_provider": config.notifications.smsProvider,
      };

      const promises = Object.entries(configMap).map(([key, value]) =>
        adminAPI.updateConfiguration(key, value, `System override for ${key}`)
      );
          ...
          <Card className="p-6 lg:col-span-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="p-3 bg-slate-50 rounded-lg flex flex-col justify-center">
                <label className="block font-medium text-slate-900 mb-2">
                  {t('admin.configuration.email_provider', 'Email Provider')}
                </label>
                <select
                  value={config.notifications.emailProvider}
                  onChange={(e) => updateNotificationValue('emailProvider', e.target.value)}
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none text-sm font-medium text-slate-700"
                >
              ...
              <div className="p-3 bg-slate-50 rounded-lg flex flex-col justify-center">
                <label className="block font-medium text-slate-900 mb-2">
                  {t('admin.configuration.sms_provider', 'SMS Provider')}
                </label>
                <select
                  value={config.notifications.smsProvider}
                  onChange={(e) => updateNotificationValue('smsProvider', e.target.value)}
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none text-sm font-medium text-slate-700"
                >
                  <option value="Twilio">Twilio Gateway</option>
                  <option value="Plivo">Plivo SMS</option>
                  <option value="Infobip">Infobip Platform</option>
