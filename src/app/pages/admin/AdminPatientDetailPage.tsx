    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-3xl font-bold text-foreground">{t("admin.patient_detail.title", "Patient Profile")}</h1>
                    <p className="text-muted-foreground">{t("admin.patient_detail.subtitle", "Detailed information for {{name}}", { name: patient.full_name || patient.name || t("common.patient", "Patient") })}</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex items-center gap-3 p-3 bg-muted/30 rounded-lg">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm font-medium text-foreground">{patient.phone || t("common.not_provided", "Not provided")}</p>
                                </div>
                            </div>
                        </div>
                        <div className="space-y-4">
                            <div className="flex items-center gap-3 p-3 bg-muted/30 rounded-lg">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm font-medium text-foreground">{patient.city || t("common.unknown", "Unknown")}</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-3 p-3 bg-muted/30 rounded-lg">
                                <Calendar className="w-5 h-5 text-primary" />
                                <div>
                                    <p className="text-xs text-muted-foreground">{t("common.registered", "Registered")}</p>
                ...
                <Card className="p-6 border border-gray-200 bg-white shadow-sm">
                    <h3 className="text-lg font-bold text-foreground mb-4">{t("admin.patient_detail.quick_stats", "Quick Stats")}</h3>
                    <div className="space-y-4">
                        <div className="flex items-center justify-between p-3 bg-blue-50 rounded-lg">
                            ...
                            <span className="text-xl font-bold text-blue-600">{data?.total_scans || 0}</span>
                        </div>
                        <div className="flex items-center justify-between p-3 bg-green-50 rounded-lg">
                            ...
                            <span className="text-xl font-bold text-green-600">{data?.total_appointments || 0}</span>
                        </div>
            ...
            <Card className="p-6 border border-gray-200 bg-white shadow-sm">
                <h3 className="text-lg font-bold text-foreground mb-4">{t("common.medical_history", "Medical History")}</h3>
