...
export default function PaymentManagementPage() {
  ...
  const { data: paymentsData, isLoading, refetch } = useQuery({
    queryKey: ['admin-payments', searchTerm, statusFilter, sortBy, page],
        ...
        {paymentsData && paymentsData.total_pages > 1 && (
          <div className="flex items-center justify-between mt-6">
            <p className="text-sm text-[#64748B]">
              Showing {(page - 1) * limit + 1} to {Math.min(page * limit, paymentsData.total)} of {paymentsData.total} payments
            </p>
            <div className="flex items-center gap-2">
              <Button
                variant="outline"
              ...
              <div className="flex items-center gap-1">
                {Array.from({ length: Math.min(5, paymentsData.total_pages) }, (_, i) => {
                  const pageNum = i + 1;
                  return (
                    <Button
                      key={pageNum}
                ...
                {paymentsData.total_pages > 5 && <span className="px-2 text-[#64748B]">...</span>}
              </div>
              <Button
                variant="outline"
                size="sm"
                onClick={() => setPage(p => Math.min(paymentsData.total_pages, p + 1))}
                disabled={page === paymentsData.total_pages}
              >
                Next
