...
import {
  ...
  Clock, Download, RefreshCw, Eye, AlertTriangle, ChevronLeft, ChevronRight
} from 'lucide-react';
...
export default function RefundManagementPage() {
  ...
  const { data: refundsData, isLoading, refetch } = useQuery({
    queryKey: ['admin-refunds', searchTerm, statusFilter, sortBy, page],
  ...
  const total_amount = stats?.total_refunded || 0;
  const processed_refunds = stats?.processed || 0;
  const pending_refunds = stats?.pending || 0;
  const failed_refunds = 0; // Backend doesn't currently return failed count in stats
                ...
                <table className="w-full">
                  <thead className="border-b bg-[#F8FAFC]">
                    <tr>
                      ...
                      <th className="text-left p-4 font-semibold text-[#0F172A]">Payment Info</th>
                  ...
                  <tbody>
                    {refunds.map((refund, index) => {
                      ...
                      const paymentInfo = (refund as any).payment_info;
                      return (
                        <motion.tr
                          key={refund.id}
                          initial={{ opacity: 0, y: 20 }}
                          ...
                          <td className="p-4">
                            {paymentInfo ? (
                              <div className="text-xs text-[#64748B]">
                                <p className="font-mono">{paymentInfo.razorpay_payment_id || 'N/A'}</p>
                                <p className="text-[10px]">{paymentInfo.razorpay_order_id || 'N/A'}</p>
                              </div>
                            ) : (
                              <span className="text-sm text-[#64748B]">N/A</span>
                            )}
                          </td>
                          <td className="p-4">
          ...
          <div className="flex items-center justify-between mt-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1">
                ...
                {total_pages > 5 && <span className="px-2 text-[#64748B]">...</span>}
              </div>
              <Button
                variant="outline"
                size="sm"
