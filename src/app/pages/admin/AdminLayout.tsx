    ...
    return (
        <div className="flex min-h-screen bg-transparent">
            ...
            <motion.aside
                ...
                className="fixed lg:sticky top-0 h-screen bg-white/70 dark:bg-[#080b11]/70 backdrop-blur-xl border-r border-gray-200/80 dark:border-white/5 shadow-sm dark:shadow-2xl z-50 flex flex-col shrink-0 overflow-hidden"
            >
                <div className="h-16 flex items-center px-6 border-b border-gray-100 dark:border-white/5 shrink-0">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#8B5CF6] to-[#6D28D9] flex items-center justify-center shadow-lg shadow-purple-900/20">
                            <Eye className="w-5 h-5 text-white" />
                        </div>
                        {sidebarOpen && <span className="text-xl font-bold text-[#0F172A] dark:text-white whitespace-nowrap">{t("admin.layout.netra_admin", "Netra Admin")}</span>}
                    </div>
                    {isMobile && (
                        <button onClick={() => setSidebarOpen(false)} className="ml-auto text-gray-500 hover:bg-gray-100 dark:hover:bg-white/5 p-2 rounded-lg">
                            <X className="w-5 h-5" />
                        </button>
                    )}
                </div>
                ...
                <nav className="flex-1 overflow-y-auto px-4 py-6 space-y-8">
                    {navItems.map((group) => (
                        <div key={group.group} className="space-y-2">
                            {sidebarOpen && <h4 className="px-4 text-[10px] font-bold text-[#64748B] dark:text-gray-600 uppercase tracking-wider mb-2">{group.group}</h4>}
                                    ...
                                    <NavLink
                                        ...
                                        className={({ isActive }) => `
                                            ...
                                            ${isActive
                                                ...
                                                : "text-[#64748B] dark:text-gray-500 hover:bg-gray-50 dark:hover:bg-white/5 hover:text-[#0F172A] dark:hover:text-white"
                                            }
                                        `}
                                    >
                ...
                <div className="p-4 border-t border-gray-100 dark:border-white/5 shrink-0">
                    <Button
                        variant="outline"
                        className="w-full flex items-center justify-start gap-3 border-gray-200 dark:border-white/10 dark:bg-transparent text-[#F43F5E] hover:bg-[#F43F5E]/5 dark:hover:bg-[#F43F5E]/10 shrink-0"
                ...
                <header className="h-16 bg-white/70 dark:bg-[#080b11]/70 backdrop-blur-md border-b border-gray-200/80 dark:border-white/5 shrink-0 z-30 px-4 sm:px-6 flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3 sm:gap-4 shrink-0">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm font-semibold text-[#0F172A] dark:text-white hidden sm:block">{t("admin.layout.super_admin", "Super Admin")}</span>
                        </div>
                    </div>
                </header>

                {/* Page Content */}
                <div className="flex-1 overflow-y-auto bg-transparent p-6 lg:p-8">
