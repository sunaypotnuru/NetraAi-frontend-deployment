...
class ApiClient {
  ...
  private getAuthHeaders(): Record<string, string> {
    ...
    // Support for DEV/Demo bypass mode
    if (import.meta.env.VITE_BYPASS_AUTH === 'true') {
      const demoEmail = localStorage.getItem('bypassEmail') || 'sunaysujsy@gmail.com';
      const demoRole = localStorage.getItem('bypassRole') || 'patient';
      headers['X-Demo-Email'] = demoEmail;
      headers['X-Demo-Role'] = demoRole;
      console.log(`[API] Using Demo Headers: ${demoEmail} (${demoRole})`);
    }
  ...
  private async makeRequest<T>(
    ...
    try {
      ...
      if (err.status) {
        ...
        if (err.status === 401) {
          // NOTE: Disabled automatic redirect for demo to prevent secondary backend 401s 
          // from kicking the user out of a valid Supabase session.
          console.warn("[API Client] 401 Unauthorized from: " + url + ". Ignoring redirect to prevent session loss.");
          /*
